...
if(clicks==1)
{
	...
	if(bus.isSelected())
	{
		...
		if(!r.next())
		{
		    ...
		    ownercompanytext.setText(r.getString("Owner_Company"));
	...
	else if(ford.isSelected())
	{
		...
		assignedlanetext.setText(r.getString("Original_Assigned_Lane"));
	    	...
	    	try {
	    		if(bus.isSelected())
				{
	    			DataBaseConnection.update("update mydb."+s+" set Plate_Num = '" +platenum+ "' ,Symbol = '" +sym+"' ,Passengers_Capacity = '"+capacity+"' ,Dicounted_Price = '" +disprice+"' ,Owner_Company = '" +ownerco+ "' "+ "   where Plate_Num= '"+platenum+"'  ");
				}
